...
{
	CHECK((alignment & (alignment - 1)) == 0, "alignment must be a power of two")
...
[[nodiscard]] inline constexpr Byte operator "" _b(size_t count)
{
	return Byte{count};
};

[[nodiscard]] inline constexpr Kilobyte operator "" _Kb(size_t count)
{
	return Kilobyte{count};
};

[[nodiscard]] inline constexpr Megabyte operator "" _Mb(size_t count)
{
